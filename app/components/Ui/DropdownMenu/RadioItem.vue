<template>
  <DropdownMenuRadioItem v-bind="forwarded" :class="styles({ class: props.class })">
    <span class="absolute left-2 flex h-3.5 w-3.5 items-center justify-center text-[#27272b]">
      <UiDropdownMenuItemIndicator>
        <Icon v-if="icon" :name="icon" class="h-4 w-4 text-[#27272b]" />
        <div v-else class="h-2 w-2 rounded-full bg-[#27272b]" />
      </UiDropdownMenuItemIndicator>
    </span>
    <slot>{{ title }}</slot>
